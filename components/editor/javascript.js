  ...
  function JSX() {
    ...
    this.$rules.jsxAttributes = [{
      ...
      onMatch(value, currentState, stack) {
        ...
        if (value.length === 2) {
          if (stack[0] === this.nextState) { stack[1] -= 1; }
          if (!stack[1] || stack[1] < 0) {
            stack.splice(0, 2);
          }
        }
...
ace.define('ace/mode/javascript', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/javascript_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/worker/worker_client', 'ace/mode/behaviour/cstyle', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  ...
  (function () {
    ...
    this.createWorker = function (session) {
      const worker = new WorkerClient(['ace'], require('brace/worker/javascript'), 'JavaScriptWorker');
      worker.attachToDocument(session.getDocument());

      worker.on('annotate', (results) => {
        const data = results.data.filter(item => item.text !== 'Missing ";" before statement');
        session.setAnnotations(data);
      });
